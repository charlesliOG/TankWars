// Author: Brevin Bell and Charles Li, November 11 2021
using System;
using System.Net;
...
namespace NetworkUtil
{
    ...
    public static class Networking
    {
        ...
        public static TcpListener StartServer(Action<SocketState> toCall, int port)
        {
            ...
            //tuple to keep track of toCall and listener
        ...
        {
            //Tuple from Start Server
            Tuple<TcpListener, Action<SocketState>> clientTuple = (Tuple<TcpListener, Action<SocketState>>)ar.AsyncState;

            Socket newClient = null;

            SocketState state;

            //check if error occurs during EndAcceptSocket on the tuple listener
            try
            {
                newClient = clientTuple.Item1.EndAcceptSocket(ar);
            }
            catch (Exception ex)
            {

                //create a SocketState to set errors
                state = new SocketState(null, null);

                state.ErrorOccurred = true;
                state.ErrorMessage = ex.Message;

                //action from tuple called whenever error occurs
                clientTuple.Item2(state);
                return;
            }

            //set the SocketState with EndAcceptSocket Socket and ToCall from StartServer
            state = new SocketState(clientTuple.Item2, newClient);

            //OnNetworkAction should be set to the delegate that was passed to StartServer
            state.OnNetworkAction = clientTuple.Item2;

            //call when connection is made
            state.OnNetworkAction(state);

            //see if errors occur when accepting other clients
            try
            {
                clientTuple.Item1.BeginAcceptSocket(AcceptNewClient, clientTuple);
            }
            catch (Exception)
            {

                state.ErrorOccurred = true;
                state.ErrorMessage = "failed to connect client";
                state.OnNetworkAction(state);
                return;
            }
        }
        ...
        public static void ConnectToServer(Action<SocketState> toCall, string hostName, int port)
        {
            ...
            try
            {
                ...
                if (!foundIPV4)
                {
                    state = new SocketState(null, null);
                    state.ErrorOccurred = true;
                    state.ErrorMessage = "Didn't find any IPV4 addresses";
            ...
            try
            {
                ...
                {
                    state.ErrorOccurred = true;
                    state.ErrorMessage = "Timeout occured";
                    toCall(state);
                    return;
                }
            }
        ...
        private static void ConnectedCallback(IAsyncResult ar)
        {
            ...
            //see if errors occur when finalizing connection
            try
            {
        ...
        {
            //see if errors occur when beginning the process of receiving data
            try
            {
            ...
            {
                state.ErrorOccurred = true;
                state.ErrorMessage = "Failed to start receiving data";
                state.OnNetworkAction(state);
            }
        }
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            SocketState state = (SocketState)ar.AsyncState;
            int numBytes = 0;
            //see if errors occur when finalizing receive
            try
            {
            ...
            //lock append line since we dont want do clients accessing it at the same time
            lock (state.data)
            {
        ...
        {
            //uses helper method SocketConnected to see if socket is closed
            if (socket.Connected)
            {
                byte[] messageBytes = Encoding.UTF8.GetBytes(data);

                //see if errors occur when beginning proccess of sending data
                try
                {
        ...
        {
            //see if errors occur when finalizing send
            try
            {
                Socket state = (Socket)ar.AsyncState;
                state.EndSend(ar);
            }
            //method should not throw even if error occurs
            catch (Exception)
            {
            }
        }
        ...
        {
            //helper method to see if socket is connected
            if (socket.Connected)
            {
